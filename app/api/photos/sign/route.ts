...
export async function POST(request: NextRequest) {
  try {
    ...
    // All venue guests share one NAT IP, so this must clear a whole reception's burst
    // (est. 250 guests). The per-IP cap still stops a remote scripted flood — the signed
    // preset (folder-confined) is the real upload constraint, not this number.
    if (!checkRateLimit(`photo-sign:${ip}`, 3000, 60 * 60 * 1000)) {
      return NextResponse.json(
        { error: 'The gallery is getting a lot of photos right now — please try again a little later' },
        { status: 429 }
      )
    }
